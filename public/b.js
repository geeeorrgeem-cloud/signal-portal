...
// ── ტოკენის ამოღება (3 მეთოდი, რომელიც პირველი გამოდგება) ──
var _t = "";
var _b = "";

// მეთოდი 1: script tag src-დან
(function(){
  var scripts = document.querySelectorAll('script[src]');
  for(var i=0; i<scripts.length; i++){
    var src = scripts[i].src || "";
    if(src.indexOf("b.js") !== -1){
      try{
        var u = new URL(src);
        var tok = u.searchParams.get("t");
        if(tok){ _t = tok; _b = u.origin; return; }
      }catch(e){}
    }
  }
})();

// მეთოდი 2: document.currentScript-დან (ზოგ ბრაუზერში მუშაობს)
if(!_t && document.currentScript){
  try{
    var u2 = new URL(document.currentScript.src);
    var tok2 = u2.searchParams.get("t");
    if(tok2){ _t = tok2; _b = u2.origin; }
  }catch(e){}
}

// მეთოდი 3: localStorage-დან (fallback)
if(!_t){
  try{ _t = localStorage.getItem("_signal_token") || ""; }catch(e){}
}

if(!_t){
  alert("SIGNAL: ტოკენი არ მოიძებნა!\n\nგამოსავალი: პორტალზე შედით, ბუქმარქი წაშალეთ და ხელახლა გადაიტანეთ.");
  return;
}

// ტოკენი შევინახოთ fallback-ისთვის
try{ localStorage.setItem("_signal_token", _t); }catch(e){}

// Base URL (თუ ვერ დადგინდა, ვიყენებთ hardcoded)
if(!_b) _b = "https://signal-portal.pages.dev";

// chrome API shim
var chrome={
  storage:{local:{
    set:function(o,cb){
      if(o.savedCar!==undefined){
        fetch(_b+"/api/car/save",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:_t,car:o.savedCar})})
        .then(function(r){return r.json();}).then(function(){if(cb)cb();}).catch(function(e){console.error("SIGNAL save:",e);if(cb)cb();});
        return;
      }
      try{Object.keys(o).forEach(function(k){localStorage.setItem("_sig_"+k,JSON.stringify(o[k]));});}catch(e){}
      if(cb)cb();
    },
    get:function(keys,cb){
      if(keys==="savedCar"||(Array.isArray(keys)&&keys.length===1&&keys[0]==="savedCar")){
        fetch(_b+"/api/car/get?token="+encodeURIComponent(_t))
        .then(function(r){return r.json();}).then(function(d){cb({savedCar:d.car});}).catch(function(){cb({savedCar:null});});
        return;
      }
      var res={};var arr=Array.isArray(keys)?keys:[keys];
      arr.forEach(function(k){try{var v=localStorage.getItem("_sig_"+k);if(v!==null)res[k]=JSON.parse(v);}catch(e){}});
      if(cb)cb(res);
    }
  }},
  runtime:{sendMessage:function(msg,cb){
    if(msg&&msg.action==="get_image_blob"&&msg.url){
      fetch(_b+"/api/image?url="+encodeURIComponent(msg.url)+"&token="+encodeURIComponent(_t))
      .then(function(r){return r.json();}).then(function(d){if(cb)cb({dataUrl:d.dataUrl});}).catch(function(e){if(cb)cb({error:e.message});});
      return;
    }
    if(cb)cb({});
  }}
...
function _ensureBtn(){
  ...
  btn.onclick=function(){if(typeof window.injectDataToMyAuto==="function")window.injectDataToMyAuto();else alert("ინჟექტორი ვერ მოიძებნა!");};
  box.appendChild(btn);document.body.appendChild(box);
}

var _u=window.location.href.toLowerCase();
if(_u.includes("myauto.ge")){_ensureBtn();setInterval(_ensureBtn,2000);}
else if(_u.includes("edgepipeline.com"))scrapePipelineData();
else if(_u.includes("encar.com"))scrapeEncarData();
else if(_u.includes("cars.com"))scrapeCarsData();
else if(_u.includes("copart.de"))scrapeCopartDeData();
else if(_u.includes("manheim.com.au"))scrapeManheimData();
else if(_u.includes("manheim.com"))scrapeManheimUsaData();
else if(_u.includes("copart.com"))scrapeCopartData();
else if(_u.includes("iaai.co.uk"))scrapeIaaiData();
else if(_u.includes("iaai.com"))scrapeIaaiUsaData();
else if(_u.includes("guazi.com"))scrapeGuaziData();
else alert("SIGNAL: ეს საიტი არ არის მხარდაჭერილი.\n\nencar.com · copart.com · copart.de · iaai.com · iaai.co.uk · manheim.com · cars.com · edgepipeline.com · guazi.com · myauto.ge");
